...
export class RoutingService {
  static connect(onClose: () => void): Promise<RoutingService> {
    return new Promise((F, R) => {
      const conn = net.createConnection(SERVICE_NAME, () => {
        F(new RoutingService(conn, onClose));
      });
  ...
  private constructor(private conn: net.Socket, private onClose: () => void) {
...
//   // Helper method to perform IPC with the Windows Service. Prompts the user for admin
//   permissions
...
//               // Don't resolve yet; we got a status change response. This can happen when
//               connecting
